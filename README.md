...
- 💰 **双维度持仓精算**: 支持多笔补仓明细录入、自动计算综合加权成本，实时计算 **今日浮盈** 与 **持仓总盈亏**。
- 🎨 **Apple Vision Pro 视觉语言**: 精细打造液态玻璃 (Liquid Glass) 材质、超大号极简数字排版与全屏无缝卡片质感。
- 🔢 **自适应字号与格式化**: 大额数字自适应缩放 (`clamp`) 与 `ellipsis` 溢出保护，支持鼠标悬停浮动查看全量精度；买入日期统一规范为 `YYYY/MM/DD` 格式。
- 💼 **持久老板键**: 股票名称和持仓术语一键切换为项目数据表达，保留数值浏览、排序和刷新，关闭弹窗后仍保持伪装状态。
- ⚡ **自动极速轮询**: 交易时间内 1 分钟自动后台更新行情，并支持右上角一键手动刷新。
- 🔍 **智能拼音搜索**: 集成腾讯财经 SmartBox，支持中文名称、股票代码及拼音缩写一键搜索添加。
