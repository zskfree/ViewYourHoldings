...
内置专属 **“摸鱼遮罩”** 模式，一键隐蔽敏感金额与持仓细节，即便身处办公室也能安心盯盘。
...
| **办公尴尬**：办公桌电脑屏幕大，被同事/领导看到持仓金额 | **摸鱼遮罩**：一键关闭/开启金额遮罩（`***`），保护个人财务隐私 |
...
- 🙈 **一键摸鱼遮罩**: 专属遮罩开关，一键将敏感数据隐藏为 `***`，轻松应对职场办公环境。
...
- **Quote Engine (`src/quote.js`)**: 负责调用腾讯财经 API，自动处理 GBK/UTF-8 字符解码与股票代码标准化。
- **Calc Engine (`src/calc.js`)**: 计算持仓成本、实时市值、当日盈亏与累计收益比例。
- **Storage Engine (`src/storage.js`)**: 基于 `chrome.storage.local` 进行配置与持仓数据的本地持久化。
- **Popup View (`popup.html` / `src/popup.js`)**: 极简 UI 响应渲染与遮罩状态控制。
